...
VS Code extension with embedded React webview: pixel art office where Cursor agents (terminals) are animated characters.
...
scripts/                      — 7-stage asset extraction pipeline
  ...
  3-vision-inspect.ts         — AI vision auto-metadata (OpenAI-compatible API)
...
**Vocabulary**: Terminal = VS Code terminal running `cursor agent`. Session = JSONL conversation file. Agent = webview character bound 1:1 to a terminal.
...
**One-agent-per-terminal**: Each "+ Agent" click → new terminal (runs `CURSOR_LAUNCH_COMMAND_TEMPLATE`, default `agent`) → immediate agent creation → 1s poll for matching `.jsonl` → file watching starts.
...
JSONL transcripts at `~/.cursor/projects/<project-hash>/agent-transcripts/<session-id>.jsonl`. Project hash = workspace path with non-alphanumeric chars → `-`.
...
## Extension packaging and testing (Cursor)

- Preferred flow: `npx @vscode/vsce@2.26.0 package` → install the `.vsix` into Cursor → reload the extension. The maintainer does not rely on `npm run build` alone for testing; vsce package is the standard. When helping with extension changes, run that command then tell the user to reload the extension.
